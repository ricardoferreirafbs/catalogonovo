@php
    ...
    $isSignature = in_array($template, ['prisma', 'impeto', 'aurea'], true);
...
<body class="catalog-body template-{{ $template }} font-{{ $tenant->themeValue('font_style','modern') }}">
    <header class="catalog-header {{ $template === 'accesso' ? 'accesso-header' : ($isMimo ? 'mimo-header' : ($isSignature ? 'signature-header '.$template.'-header' : '')) }} container">
        ...
        <nav aria-label="Navegação principal">
            ...
            @if($menuItems->isEmpty() && $menuCategories->isEmpty())<a href="{{ route('catalog.index') }}#catalogo">Catálogo</a>@if($template==='accesso' || $isSignature)<a href="{{ route('catalog.index') }}#experiencia">Experiência</a><a href="{{ route('catalog.index') }}#como-pedir">Como pedir</a>@elseif($isMimo)<a href="{{ route('catalog.index') }}#como-pedir">Como funciona</a><a href="{{ route('catalog.index') }}#sobre">Sobre</a>@endif @endif
            @if($whatsapp)<a class="header-cta" href="{{ $whatsapp }}" target="_blank" rel="noopener">{{ $isMimo ? 'Pedir orçamento' : ($template === 'impeto' ? 'Falar com consultor' : 'Falar no WhatsApp') }} <span>↗</span></a>@endif
        </nav>
    </header>
    <main>@yield('content')</main>
    <footer class="catalog-footer {{ $template === 'accesso' ? 'accesso-footer' : ($isMimo ? 'mimo-footer' : ($isSignature ? 'signature-footer '.$template.'-footer' : '')) }}">
        @if($isMimo)
