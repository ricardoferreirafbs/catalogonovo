...
<body class="catalog-body template-{{ $template }} font-{{ $tenant->themeValue('font_style','modern') }}">
    <header class="catalog-header {{ $template === 'accesso' ? 'accesso-header' : '' }} container">
        <a href="{{ route('catalog.index') }}" class="brand" aria-label="Página inicial de {{ $tenant->name }}">
            @if($tenant->logo_path)<img src="{{ Storage::disk('uploads')->url($tenant->logo_path) }}" alt="{{ $tenant->name }}">@else<span class="brand-mark">{{ mb_substr($tenant->name,0,2) }}</span><span>{{ $tenant->name }}</span>@endif
        </a>
        ...
        <nav aria-label="Navegação principal">
            ...
            @if($menuItems->isEmpty() && $menuCategories->isEmpty())<a href="{{ route('catalog.index') }}#catalogo">Catálogo</a>@if($template==='accesso')<a href="{{ route('catalog.index') }}#experiencia">Experiência</a><a href="{{ route('catalog.index') }}#como-pedir">Como pedir</a>@endif @endif
            @if($whatsapp)<a class="header-cta" href="{{ $whatsapp }}" target="_blank" rel="noopener">Falar no WhatsApp <span>↗</span></a>@endif
        </nav>
    </header>
    <main>@yield('content')</main>
    <footer class="catalog-footer {{ $template === 'accesso' ? 'accesso-footer' : '' }}">
    ...
    @if($whatsapp)<a class="floating-whatsapp" href="{{ $whatsapp }}" target="_blank" rel="noopener" aria-label="Fale conosco no WhatsApp"><span>WA</span> Fale conosco</a>@endif
</body>
</html>
